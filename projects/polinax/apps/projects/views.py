...
def delete(request, slug, redirect_url=None):
    ...
    if request.user.is_authenticated() and request.method == "POST" and request.user == project.creator:
        ...
        project.save()
        # TODO: move this to Group.save
        request.user.message_set.create(message="Project %s deleted." % project)
        if notification:
            notification.send(project.members.all(), "projects_deleted", {"project": project})
...
def your_projects(request, template_name="projects/your_projects.html"):
    return render_to_response(template_name, {
        "projects": Project.objects.filter(deleted=False, members=request.user).order_by("name"),
    }, context_instance=RequestContext(request))
...
def members_status(request, slug, form_class=AwayForm,
    ...
    is_member = project.has_member(request.user)
    try:
        project_member = Membership.objects.get(group=project, user=request.user)
    ...
    active_members = Membership.objects.filter(away=False)
    away_members = Membership.objects.filter(away=True).order_by('away_since')
